...
是服务器发送到浏览器的一小段数据，会在浏览器下次向同一服务器发送请求时带上一起发送给服务器。

基于 HTTP 规范，用于识别用户，诞生之初是为了解决 HTTP 的无状态请求，用来记录用户的一些状态。
...
![img](webp-20201222210433409)

情况2：协商缓存失效，返回 `200` 和请求结果

![img](webp-20201222210503005)
...
从页面中接受事件的顺序，IE（事件冒泡流）、NetScape（事件捕获流） 提出的事件流完全相反。
...
> addEventListener(EventName, Callback, BoolVal)
>
> 为DOM2级事件处理程序，它接受三个参数：要处理的事件名、作为事件处理程序的函数和一个布尔值。
>
> 最后这个布尔值参数如果是 true，表示在捕获阶段调用事件处理程序；如果是 false，表示在冒泡阶段调用事件处理程序。

### NetScape（事件捕获流）

从文档节点开始，逐渐向下传播至最具体元素。

![img](1-20201222214616068)
